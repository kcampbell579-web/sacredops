...
export type Projector = {
  ...
  // Empty the projected store's table(s) (child rows cascade).
  clear: () => Promise<void>;
};
...
export const projectors: Record<string, Projector> = {
  ...
  sacredops_submissions: {
    ...
    write: async (value) => {
      ...
      // from clobbering each other. Rows are independent, so upsert in parallel.
      await Promise.all(
        items.map((s) => {
          const id = str(s.id);
          ...
          return prisma.submission.upsert({ where: { id }, update: data, create: { id, ...data } });
        })
      );
    },
    clear: async () => {
...
projectors.sacredops_inspections = {
  ...
  write: async (value) => {
    ...
    // items are replaced wholesale when that inspection is (re)written. Each
    // inspection's transaction is independent, so run them in parallel.
    await Promise.all(
      items.map((ins) => {
        const id = str(ins.id);
        if (!id) return Promise.resolve();
        ...
        const checkRows = (Array.isArray(checks) ? checks : []).map((pair: unknown, idx: number) => {
          ...
          return { inspectionId: id, label: str(p?.[0]) ?? "", result: str(p?.[1]) ?? "", position: idx };
        });
  ...
  write: async (value) => {
    ...
    await Promise.all(
      items.map((p) => {
        ...
        return prisma.project.upsert({ where: { id }, update: data, create: { id, ...data } });
      })
    );
  },
  clear: async () => {
